...
if ! xcodebuild -project Corbie.xcodeproj -scheme Corbie -configuration Release \
    ...
    tail -n 40 "$LOG" >&2
    echo "check_release_debug_code: the Release build failed, full log in $LOG" >&2
    exit 1
fi
...
if ! grep -q 'scan_debug_markers: ok' "$LOG"; then
    echo "check_release_debug_code: the Release build did not run the marker scan phase, full log in $LOG" >&2
    exit 1
fi

scripts/scan_debug_markers.sh "$APP"
