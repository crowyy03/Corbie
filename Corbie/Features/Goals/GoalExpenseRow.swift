import CorbieCore
import SwiftUI

struct GoalExpenseRow: View {
    let expense: GoalExpenseDTO
    let goalCurrency: String
    let memberColor: Color
    let memberName: String

    var body: some View {
        Card {
            HStack(alignment: .top, spacing: CorbieSpacing.s) {
                MemberDot(color: memberColor, accessibilityLabel: memberName)
                    .padding(.top, CorbieSpacing.xxs)
                VStack(alignment: .leading, spacing: CorbieSpacing.xxs) {
                    HStack(alignment: .firstTextBaseline, spacing: CorbieSpacing.xs) {
                        Text(Money(amount: expense.amount, currency: expense.currency).formatted())
                            .corbieBody()
                            .fontWeight(.semibold)
                            .foregroundStyle(CorbieColorPalette.text)
                        if expense.isConverted {
                            Text(Money(amount: expense.amountInGoalCurrency, currency: goalCurrency).approximate())
                                .corbieMono()
                                .foregroundStyle(CorbieColorPalette.text2)
                        }
                    }
                    if let note = expense.note, note.isEmpty == false {
                        Text(note)
                            .corbieCaption()
                            .foregroundStyle(CorbieColorPalette.text2)
                    }
                    if let date = expense.date {
                        Text(date.formatted(date: .abbreviated, time: .omitted))
                            .corbieMono()
                            .foregroundStyle(CorbieColorPalette.text2)
                    }
                }
                Spacer(minLength: 0)
            }
        }
        .accessibilityElement(children: .combine)
    }
}

#if DEBUG
#Preview {
    VStack(spacing: CorbieSpacing.m) {
        GoalExpenseRow(
            expense: GoalExpenseDTO(
                id: UUID(),
                amount: 210,
                currency: "EUR",
                fxRateToGoalCurrency: 1.09,
                amountInGoalCurrency: 228.9,
                note: "deposit",
                date: Date()
            ),
            goalCurrency: "USD",
            memberColor: MemberColorKey.p2.color,
            memberName: "Sofia"
        )
        GoalExpenseRow(
            expense: GoalExpenseDTO(
                id: UUID(),
                amount: 640,
                currency: "USD",
                amountInGoalCurrency: 640,
                note: "flights",
                date: Date()
            ),
            goalCurrency: "USD",
            memberColor: MemberColorKey.p1.color,
            memberName: "you"
        )
    }
    .padding(CorbieSpacing.l)
    .frame(maxWidth: .infinity, maxHeight: .infinity)
    .background(CorbieColorPalette.bg)
}
#endif
