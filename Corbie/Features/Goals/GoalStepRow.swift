...
struct GoalStepRow: View {
    ...
    private var detailsLabel: String {
        [String(format: String(localized: "goals.step.action.edit"), step.title), assigneeName, due?.text]
            .compactMap { $0 }
            .joined(separator: ", ")
    }
    ...
    private var noteDisclosure: some View {
        ...
        .accessibilityLabel(
            Text(
                isNoteShown
                    ? String(localized: "goals.step.action.note.hide")
                    : String(localized: "goals.step.action.note.show")
            )
        )
    }
}
...
#Preview {
    ...
    return VStack(spacing: CorbieSpacing.s) {
        GoalStepRow(
            step: dated,
            due: goalStepDue(for: dated, now: Date()),
        ...
        GoalStepRow(
            step: GoalStepDTO(id: UUID(), title: "Drop the suit at the cleaner", isDone: true),
            due: nil,
            assigneeColor: nil,
            assigneeName: nil,
            isReadOnly: false,
