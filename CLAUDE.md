...
## Hard constraints
- iOS 17.2+, Swift 5.10+, SwiftUI only (no UIKit views except where SwiftUI has no API: EventKit UI, share extension host).
...
- Purchases: StoreKit 2 only. Always pass `appAccountToken = Space.id`. The trial is an Apple introductory offer (14 days), never a self-managed trial clock. Premium gate resolves from the local StoreKit entitlement, the server entitlement by `spaceId`, and the CloudKit-cached status, in that order. Never gate the non-paying partner on `Transaction.currentEntitlements`. A revocation counts only for a transaction whose `appAccountToken` is this `Space.id` and only if it is that space's latest transaction, on the client and on the server. The device's own StoreKit transactions count whatever their environment, because StoreKit only hands a build the transactions of the environment it runs in (that is how an App Review purchase, made in the sandbox from a production-signed build, unlocks). A sandbox transaction never leaves the device as paid: only production transactions reach the CloudKit Space record, the server answers each build from its own environment's rows, and the widgets trust this device's snapshot only when it carries the widget's own environment.
- Monetization flag: a client that never fetched `/config` and a server with no `app_config` row both count as on, because v1 is paid from day one. The live row is flipped by the founder at launch, never by a migration.
...
- Every new entity or field needs a migration mapping, a rebuild of the widget and share targets and a note in `docs/DECISIONS.md`.
...
- Don't add free tier beyond Calendar, Today (view), Weekly recap, viewing everything already made, answering and revealing votes, and the DaysTogether and Countdown widgets.
- Don't put padlock icons on rows in read-only mode and never hide the user's own data behind the paywall: gate the action, not the view. Calendar stays fully editable from every entry point, including Add a date on Today. Notification actions and widget intents are gated like buttons: in read-only they write nothing and open the app on the paywall.
...
- `Packages/CorbieCore` must also compile and test on macOS: `cd Packages/CorbieCore && swift build && swift test`. Platforms: `.iOS("17.2"), .macOS("14.2")`. Wrap UIKit-only code in `#if canImport(UIKit)` and provide an AppKit or platform-neutral path.
