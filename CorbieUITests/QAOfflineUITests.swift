...
final class QAOfflineUITests: XCTestCase {
    ...
    func testAnUnreachableServerLeavesTheInviteScreenUsable() throws {
        ...
        XCTAssertTrue(app.buttons["Try again"].exists, "a failed invite call offers no retry")
        saveScreenshot(app, named: "offline_invite_failed")
        XCTAssertTrue(app.buttons[QAText.later].exists, "the invite screen cannot be left")
        app.buttons[QAText.later].tap()
        XCTAssertTrue(
