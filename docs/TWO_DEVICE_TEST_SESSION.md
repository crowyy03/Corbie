...
| 6 | Build: `xcodegen generate`, open `Corbie.xcodeproj`, scheme **Corbie**, Run (Debug) on phone A, then on phone B, from the same checkout state. Never Profile, Archive or run the Release configuration on these phones until the Production CloudKit schema is deployed (`docs/RELEASE_CHECKLIST.md`, "CloudKit schema deployed"): a Release build writes to Production. From 2026-09-26 on, a code made on one phone cannot be redeemed on a phone whose build writes to the other environment; it answers "This code comes from a different build of Corbie. Both phones need the same one." Write down `git rev-parse --short HEAD` and whether the tree had local changes. `git log --oneline` must contain `63e4af1` and `2e15dfa`. | Both phones must run the same code. Steps 8, 8b, 13, 13b, 14 and 16 describe the build with the persistence fixes, steps 4, 6e, 8, 9, 9e, 12a and 12c the build with fix round B. |
...
| 10 | CloudKit environment: set by the build configuration since 2026-09-26, not by signing. Debug uses **Development**, Release uses **Production** (`CORBIE_CLOUDKIT_ENVIRONMENT` in `project.yml`, carried into all three entitlements files and into `CorbieCloudKitEnvironment` in the Info.plist). Developer, section Server, shows `cloudkit development` on a Debug build. In the Dashboard, always pick **Development**. | Data from this session never appears in Production. See section 2. |
...
| 12 | Server: live at `powtuiqqagdoiuqjeebr`, nothing to do for this session. Invite codes and redeem have worked since the deploy of 2026-09-12 (`server/DEPLOY.md`). Migration 0010 and the `invite` and `invite-redeem` functions that check the CloudKit environment and the iCloud account went live on 2026-09-26; a build older than that sends neither and is not checked. `POST /session` has never run with a real Apple token before: step 1 is its first real call. | |
| 13 | Monetization is off on the server, so both phones have full access. Server check: `curl -s https://powtuiqqagdoiuqjeebr.supabase.co/functions/v1/config` prints `{"monetizationEnabled":false}` (checked on 2026-09-26). A value the app never fetched counts as **on** (`MonetizationFlagStore.isEnabled`), so a phone offline at first launch starts in the paid flow until it reaches the server. In-app check on each phone: Us pill, Shared settings, About, "Developer" (Debug builds only). The Status section must read `monetization: off, server says off` and `forced state: none, this is the real one`; if not, tap "Follow the server flag" and "Follow the real subscription". For the pairing steps touch nothing else. To look at the paywall or a subscription state later, use the rows under Subscription: each of them forces monetization on by itself, and "Follow the server flag" turns it back off. | With the flag off, Settings has no "Subscription" section, the Shared settings tile reads "names, dates and notifications", no trial offer screen appears, and no widget shows "Unlock in Corbie". Any of those appearing means the flag is not off on that phone. |
...
- **A TestFlight, App Store, Archive or Release build** uses Production, also when Xcode installs it. Until the schema is deployed there
