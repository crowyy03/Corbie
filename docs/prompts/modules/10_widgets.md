...
Read `docs/01a_SPEC_AMENDMENT_01.md` (widget section) first, then spec §8, architecture §7, §18 (5–6).
...
4a. **Amendment changes**: `PlanProgress` → `GoalProgress`; `Shopping` reads tasks from the pinned Shopping `TaskFolder`; `Tasks` and `FreeTasks` filter to `folderId == nil` by default; `OurDay` large **must use `TodayFeedProvider`** (Module 16) so widget and Today screen never disagree. Optional if time allows: `FreeSlots` medium showing the next two shared free windows (Module 17).
