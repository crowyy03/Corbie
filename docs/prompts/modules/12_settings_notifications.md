...
Read `docs/01a_SPEC_AMENDMENT_01.md` first, then spec §6.11, §9, §15; architecture §8, §14, §18 (13–14).

## Do
1. `SettingsView` sections per spec: You (name, color, birthday), Partner, Our dates (together since, wedding), Notifications (**10** toggles — the 10th is `Weekly recap`, Module 18 — stored in Member.notificationPrefs), `Share my busy times` toggle (default **off**; turning it off deletes that member's device-sourced `BusyInterval` records immediately — see Module 17), Space currency, Import from iPhone Calendar (Module 05), Subscription (status/manage/restore), Privacy (copy + links), Export my data (JSON via ShareLink), Leave space, Delete account, Theme (system/light/dark), version.
...
## Verify
All 10 notification types fire in the right situations with prefs respected; export produces valid JSON; delete account fully resets.
