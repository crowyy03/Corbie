...
| "Check Release carries no debug-only code" build phase and `scripts/check_release_debug_code.sh` | Every non-Debug build, and by hand | No screenshot-mode or developer-tool symbol, string or file in the Release app or its extensions: `strings` and `nm` on the binaries, the bytes of every file as UTF-8 and UTF-16, and the file names. See `docs/RELEASE_CHECKLIST.md`, Release configuration. |
| "Check the app is iPhone only" build phase, `scripts/check_iphone_only.sh` | Every build | `UIDeviceFamily` is `[1]` in the app and both extensions, so App Store Connect does not ask for iPad screenshots. Deleting `TARGETED_DEVICE_FAMILY: "1"` from `project.yml` would make XcodeGen write `1,2`. |
