...
@Suite struct SpaceRepositoryTests {
    ...
    @Test func deletingSpaceRemovesEveryChild() async throws {
        ...
        _ = try await repositories.plans.addExpense(planId: plan.id, draft: PlanExpenseDraft(amount: 10, currency: "USD"))
        _ = try await repositories.plans.addStep(planId: plan.id, draft: PlanStepDraft(title: "Pack"))
        ...
        let entities = [
            ...
            "PlanExpense", "PlanStep", "ChecklistList", "ListItem", "BusyInterval", "Capsule",
            "Vote", "Person", "GiftIdea"
        ]
